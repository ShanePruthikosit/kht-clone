'''
create_district_table.py
A python program to create a district table and
insert data from a CSV file into the table.
Also updates the table with new data from the CSV file.
Data is about districts in Mae Hong Son province.
...
'''
Function to create the district table
'''
    ...
    try:    
        ...
        with psycopg2.connect(**params) as connection:
            with connection.cursor() as crsc:
                ...
                input_file_path = get_file_path('Schools_Towns_Districts_001.csv', 'Data')
                ...
                if new_data.empty:
                    print('\nNo new rows to add to the district table.')
                    return
                else:
                    print('\nAdding new rows to the district table...')
                    num_rows_added = len(new_data)
                    print(f'{num_rows_added} total rows added to the district table.')
